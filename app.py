...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:si364@localhost/ibilichw5"
## Provided:
...
class UpdateInfoForm(FlaskForm):
    UpdateItems = TextAreaField("What is the new priority number of this item?", validators=[Required()])
    submit = SubmitField('Update')
...
def update(item):
    ...
    if form.validate_on_submit():
        Update_Items = form.UpdateItems.data
        i = TodoItem.query.filter_by(id = item).first()
        i.priority = Update_Items
        db.session.commit()
        flash("Updated rating of: " + " '" + i.description +  "' ")
        return redirect(url_for('all_lists'))
